...
if __name__ == "__main__":
	...
	# os.environ.set('MASTER_ADDR', '29655')
	...
	class LitUNet(pl.LightningModule):
		def __init__(self):
			...
			self.model = self.fsdp_model_init()
		...
		def configure_optimizers(self):
			optimizer = torch.optim.Adam(self.trainer.model.parameters(),lr=configs["net_params"]['lr'], betas=(0.9, 0.999), eps=1e-08, weight_decay=configs["net_params"]['reg'])
			return optimizer
		...
		def configure_sharded_model(self):
			#self.model = Lpt2NbodyNet(BasicBlock)#self.net() #<however you instantiate the model here>
			self.fsdp_model_init()

		def fsdp_model_init(self)->nn.Module:
			my_auto_wrap_policy = functools.partial(
				size_based_auto_wrap_policy, min_num_params=100_000
				)
			prec = torch.bfloat16 #if args.precision == 'float32' else torch.bfloat16
			...
			model = FSDP(Lpt2NbodyNet(BasicBlock),
	...
	if configs["is_train"]:
		...
		fsdp_strategy = FSDPStrategy(cpu_offload=CPUOffload(True), #True or False
		...
		trainer = pl.Trainer(num_nodes=1, profiler=SimpleProfiler(), strategy = fsdp_strategy, accelerator= 'gpu', devices=-1, max_epochs=10)
